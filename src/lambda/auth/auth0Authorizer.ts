...
const jwksUrl = 'https://koichi-auth0.jp.auth0.com/.well-known/jwks.json';
const kid = 'S5tDG7FTy__bs-VJBXLcN';
...
async function verifyToken(authHeader: string): Promise<JwtPayload> {
  ...
  const key = await getSigningKey(jwksUrl);
  return verify(token, key.publicKey, { algorithms: ['RS256'] }) as JwtPayload;
}

const getSigningKey = async (jwksUrl: any) => {
